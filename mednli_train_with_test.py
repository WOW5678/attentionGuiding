...
def test(classifier, test_loader, args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(tqdm(test_loader)):
            ...
            if args.attention_pca == True:
                # 将数据保存下来
                ehr_attentions = ehr_attentions.detach().cpu().numpy()
                with open(os.path.join(args.project_dir, '%s_attention.pkl' % args.loss_type), 'wb') as f:
                    pickle.dump(ehr_attentions, f)
                break
            ...
            test_batches_score.append(prediction.detach().cpu().numpy())
        ...
        test_batches_score = np.concatenate(test_batches_score, axis=0)
